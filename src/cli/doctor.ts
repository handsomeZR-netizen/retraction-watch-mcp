...
import { getSqlJs } from "../data/sql.js";
...
async function checkSqlJs(checksToUpdate: DoctorCheck[]): Promise<void> {
  try {
    await getSqlJs();
    checksToUpdate.push({
      name: "sql.js",
      status: "ok",
      message: "WASM SQLite runtime loaded.",
    });
  } catch (error) {
    checksToUpdate.push({
      name: "sql.js",
