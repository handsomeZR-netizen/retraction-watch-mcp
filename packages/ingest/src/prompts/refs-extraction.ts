export const REFS_EXTRACTION_SYSTEM_PROMPT = `你是一个学术参考文献结构化抽取引擎。输入是从论文 References 段落抽取的若干条参考文献原文（每条用 <ref index="N">...</ref> 包裹，可能是中文或英文，可能包含期刊缩写、年份、卷期、页码、DOI、URL 等噪音）。
...
5. doi：仅当原文显式出现 10.\\d{4,9}/.+ 模式时填写；不要从 URL 推断。
...
export const HEADER_PARSE_SYSTEM_PROMPT = `你是一个学术稿件首页元数据抽取引擎。输入是从论文第一页抽取的纯文本（包含标题、作者、机构、邮箱、ORCID 等），噪音较多。请抽出结构化元数据。
