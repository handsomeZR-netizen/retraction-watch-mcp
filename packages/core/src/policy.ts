...
export type PolicyMode = "balanced" | "strict" | "custom";

export interface PolicyThresholds {
  likelyMatch: number;
  possibleMatch: number;
  referenceConfirmed: number;
  referenceLikely: number;
  referencePossible: number;
  referenceTitleJaccardLikely: number;
  referenceTitleJaccardPossible: number;
}
...
export interface PolicyWeights {
  ...
  referenceDoiExact: number;
  referencePmidExact: number;
  referenceTitleHigh: number;
  referenceTitlePartial: number;
  referenceAuthorOverlap: number;
  referenceAuthorSurnameOnly: number;
  referenceYearMatch: number;
  referenceYearConflictPenalty: number;
  referenceJournalMatch: number;
}
...
export const BALANCED_POLICY: ScreeningPolicy = {
  ...
  thresholds: {
    likelyMatch: 0.7,
    possibleMatch: 0.35,
    referenceConfirmed: 0.95,
    referenceLikely: 0.7,
    referencePossible: 0.4,
    referenceTitleJaccardLikely: 0.55,
    referenceTitleJaccardPossible: 0.3,
  },
  weights: {
    doiExact: 1,
    pmidExact: 1,
    ...
    referenceDoiExact: 1,
    referencePmidExact: 1,
    referenceTitleHigh: 0.55,
    referenceTitlePartial: 0.3,
    referenceAuthorOverlap: 0.25,
    referenceAuthorSurnameOnly: 0.12,
    referenceYearMatch: 0.1,
    referenceYearConflictPenalty: -0.15,
    referenceJournalMatch: 0.08,
  },
  safety: {
...
export const STRICT_POLICY: ScreeningPolicy = {
  ...
  thresholds: {
    ...
    likelyMatch: 1.01,
    possibleMatch: 1.01,
    referenceLikely: 1.01,
    referencePossible: 1.01,
  },
  safety: {
...
function readMode(value: unknown, fallback: PolicyMode): PolicyMode {
  return value === "balanced" || value === "strict" || value === "custom" ? value : fallback;
}
